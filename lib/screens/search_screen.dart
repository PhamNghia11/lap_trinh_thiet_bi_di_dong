  ...
  Future<void> _runSearch() async {
    ...
    try {
      ...
      var filtered = _filter.genreId == null
          ...
          : results.where((movie) => movie.genres.contains(_filter.genreLabel)).toList();
      if (_filter.minRating > 0) {
        filtered = filtered.where((movie) => movie.rating >= _filter.minRating).toList();
      }
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SizedBox(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  if (_filter.minRating > 0) _chip('Từ ${_filter.minRating.toStringAsFixed(1)} ★'),
                  if (_filter.sortBy != 'popularity.desc') _chip(_filter.sortLabel),
                ],
              ),
            ),
          ...
          Padding(
            ...
            child: Text(
              _controller.text.trim().isEmpty ? 'Khám phá phim' : '${_results.length} kết quả',
          ...
          Expanded(
            child: _results.isEmpty && !_loading
                ? const Center(child: Text('Không tìm thấy phim phù hợp. Hãy thử nới bộ lọc.', style: AppTheme.bodyText, textAlign: TextAlign.center))
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        ...
                        title: Text(movie.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
                        subtitle: Text('${movie.year == 0 ? 'Chưa rõ năm' : movie.year}  •  ${movie.rating.toStringAsFixed(1)} ★\n${movie.genreText}', maxLines: 2, overflow: TextOverflow.ellipsis, style: AppTheme.smallText),
